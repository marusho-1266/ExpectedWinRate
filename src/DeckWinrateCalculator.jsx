  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white">
      ...
      <div className="bg-gray-50 p-6 rounded-lg mb-6">
        <h2 className="text-xl font-semibold mb-4 text-gray-800">デッキマッチアップ設定</h2>
        
        <div className="space-y-4">
          {matchups.map((matchup, index) => (
            <div key={matchup.id} className="flex gap-4 items-center bg-white p-4 rounded border">
      ...
      {showResults && results && (
        <div className="bg-blue-50 p-6 rounded-lg">
          <h2 className="text-xl font-semibold mb-4 text-blue-800">計算結果</h2>
          
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-white p-4 rounded-lg">
              <h3 className="text-lg font-semibold mb-3 text-gray-800">期待勝率</h3>
              <div className="text-3xl font-bold text-blue-600 mb-4">
                {results.expectedWinrate}%
              </div>
              
              <h4 className="font-semibold mb-2 text-gray-700">マッチアップ</h4>
              <div className="space-y-1 text-sm">
                {results.matchups.map((matchup, index) => (
                  <div key={index} className="flex justify-between">
                    <span>{matchup.deckName}</span>
                    <span>{parseFloat(matchup.usageRate).toFixed(1)}% ({matchup.advantage}%)</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white p-4 rounded-lg">
              <h3 className="text-lg font-semibold mb-3 text-gray-800">戦数別期待勝利数</h3>
              <div className="space-y-2">
                {results.battleResults.slice(0, 8).map((result) => (
                  <div key={result.battles} className="flex justify-between items-center py-1 border-b border-gray-100">
                    <span className="font-medium">{result.battles}戦</span>
                    <div className="text-right">
                      <span className="text-blue-600 font-bold">{result.rounded}勝</span>
                      <span className="text-gray-500 text-sm ml-2">({result.expectedWins})</span>
                    </div>
                  </div>
                ))}
              </div>
